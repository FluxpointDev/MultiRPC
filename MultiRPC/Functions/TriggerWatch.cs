    ...
    public static class TriggerWatch
    {
        ...
        static TriggerWatch()
        {
            if (File.Exists(FileLocations.ProfilesFileLocalLocation))
            {
                ...
                {
                    MasterCustomPage.Profiles = (Dictionary<string, CustomProfile>)App.JsonSerializer.Deserialize(reader,
                        typeof(Dictionary<string, CustomProfile>));
                }
            }
            else
            {
                MasterCustomPage.Profiles = new Dictionary<string, CustomProfile>
                {
